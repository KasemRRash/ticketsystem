...
#kundenname=$(echo $QUERY_STRING | grep -o 'kundenname=[^&]*' | sed 's/^kundenname=//g')
...
#beschreibung=$(echo $QUERY_STRING | grep -o 'beschreibung=[^&]*' | sed 's/^beschreibung=//g' | sed 's/+/ /g')
...
echo "<!DOCTYPE html>
      <html>
        ...
        <a href="ts_admindashboard.sh">Zum Dashboard</a>
        </body>
        </html>"
